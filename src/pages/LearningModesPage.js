    ...
    <div className='ModesPage'>
      ...
      <div className='contentWrapper'>
        <div className='modesContent'>
          <h1 className='modesTitle' data-testid='modes-title'>Tryby nauki</h1>

          {learningModes.map((mode, index) => (
            <div key={index} className="modes">
              <button onClick={() => handleModeClick(index)} className='buttonModes' data-testid={`mode-button-${index}`}>
                {mode.name} {expandedIndex === index ? '-' : '+'}
              </button>
              {expandedIndex === index && (
                <>
                  <div className='modeDescription' data-testid={`mode-description-${index}`}>{mode.description}</div>
                  <img className='modesImages' src={mode.imageUrl} alt={mode.name} data-testid={`mode-image-${index}`} />
                </>
              )}
            </div>
          ))}
